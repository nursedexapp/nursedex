...
export default async function CategoryArchivePage({
  ...
  if (!archive) {
    // A renamed/merged category: redirect to its current slug if it still
    // resolves, otherwise 404.
    const to = await getLiveTaxonomyRedirect("category", slug);
    if (to) permanentRedirect(`/blog/category/${to}`);
    notFound();
  }
