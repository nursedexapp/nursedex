...
function MockInput({ label, placeholder, rightIcon, secondaryLabel }: { label: string; placeholder: string; rightIcon?: React.ReactNode; secondaryLabel?: React.ReactNode }) {
  return (
    <div>
      <div className="flex items-center justify-between mb-1">
        <label className="text-xs font-medium text-soft-black">{label}</label>
        {secondaryLabel}
      </div>
      <div className="h-10 rounded-lg border border-[#C8D2D0] px-3 flex items-center justify-between">
        <span className="text-sm text-soft-black-light">{placeholder}</span>
        {rightIcon}
      </div>
    </div>
  );
...
function SectionLabelLight({ children }: { children: React.ReactNode }) {
  return (
    <p className="font-body text-xs uppercase tracking-[0.4em] text-sage/40 mb-4">
      {children}
    </p>
  );
}

function DecisionList({ items }: { items: { decision: string; reasoning: string }[] }) {
  return (
    <div className="divide-y divide-sage/20">
      {items.map((item) => (
        <div
          key={item.decision}
          className="grid grid-cols-1 lg:grid-cols-[1fr_2fr] gap-4 lg:gap-16 py-10 first:pt-0 last:pb-0 items-baseline"
        >
          <h3 className="font-heading text-lg text-teal-dark">
            {item.decision}
          </h3>
...
function DecisionListLight({ items }: { items: { decision: string; reasoning: string }[] }) {
  return (
    <div className="space-y-12">
      {items.map((item) => (
        <div key={item.decision}>
          <h3 className="font-heading text-xl text-warm-white mb-3">
            {item.decision}
          </h3>
          <p className="font-body text-sage-light text-sm leading-relaxed max-w-2xl">
            {item.reasoning}
          </p>
        </div>
      ))}
...
function MockAnnotations({ items }: { items: { label: string; rule: string }[] }) {
  return (
    <div className={`grid sm:grid-cols-2 lg:grid-cols-${items.length > 3 ? 4 : items.length} gap-8 mt-10`}>
      {items.map((item) => (
        <div key={item.label}>
          <p className="font-body text-[10px] uppercase tracking-widest text-sage/50 mb-2">
            {item.label}
          </p>
...
export default function AuthDesignRationale() {
  return (
    <div className="min-h-screen bg-warm-white">
      ...
      {/*  COVER                                                        */}
      {/* ══════════════════════════════════════════════════════════════ */}
      <section className="relative bg-teal-dark overflow-hidden flex flex-col items-center justify-center min-h-[60vh] px-6 text-center">
        ...
        <div className="relative z-10 flex flex-col items-center gap-6">
          ...
          <h1 className="font-heading text-5xl sm:text-6xl lg:text-7xl text-warm-white leading-tight">
            Authentication
          </h1>
          <p className="font-body text-sage text-base sm:text-lg max-w-md">
            The reasoning behind every decision in the NurseDex login, signup,
            and password recovery experience.
          </p>
          <div className="w-10 h-0.5 bg-sage/50 rounded-full mt-2" />
        </div>
      </section>
      ...
      {/*  PRINCIPLE                                                    */}
      {/* ══════════════════════════════════════════════════════════════ */}
      <section className="bg-cream px-6 sm:px-12 lg:px-24 py-24">
        <div className="max-w-4xl mx-auto">
          <blockquote className="font-heading text-4xl sm:text-5xl lg:text-6xl italic text-soft-black leading-[1.08]">
            Authentication is the
            <br />
            first act of trust.
          </blockquote>
          <p className="font-body text-base text-soft-black-light leading-relaxed mt-8 max-w-lg">
            For a healthcare platform, the login screen is not just a gate. It is
            the first moment a family or nurse decides whether NurseDex feels
      ...
      {/*  1. LAYOUT                                                    */}
      {/* ══════════════════════════════════════════════════════════════ */}
      <section className="bg-warm-white px-6 sm:px-12 lg:px-24 py-32">
        <div className="max-w-5xl mx-auto">
          <SectionLabel>Layout</SectionLabel>
          <h2 className="font-heading text-3xl sm:text-4xl text-teal-dark mb-16">
      ...
      {/*  2. LOGIN MOCKUP                                              */}
      {/* ══════════════════════════════════════════════════════════════ */}
      <section className="bg-soft-black px-4 sm:px-8 lg:px-16 py-16">
        <div className="max-w-5xl mx-auto">
          <p className="font-body text-xs uppercase tracking-[0.4em] text-sage/40 mb-10 text-center">
            Login / Desktop
          </p>
            ...
            <div className="grid lg:grid-cols-2 min-h-[520px]">
              ...
              <div className="relative bg-teal px-10 py-16 flex flex-col justify-center overflow-hidden">
                ...
                <div className="relative max-w-xs">
                  ...
                  <div className="mt-10 pt-4 border-t border-warm-white/10">
                    <p className="text-sage-light/60 text-[10px] leading-relaxed italic">Built for the Long Island care community. Connecting families with trusted, verified nurses since 2026.</p>
                  </div>
                </div>
              </div>
              {/* Form panel */}
              <div className="flex items-center justify-center px-10 py-16">
                <div className="w-full max-w-sm">
                  ...
                  <p className="text-xs text-soft-black-light mb-6">Your Long Island care community is waiting. <span className="text-teal font-medium underline">New here? Create an account</span></p>
                  <div className="w-full h-10 rounded-lg border border-sage-dark/50 flex items-center justify-center gap-2 mb-5">
                  ...
                  <div className="space-y-3.5">
                    ...
                    <MockInput
                      label="Password"
                      placeholder="........"
                      rightIcon={<Eye className="h-3.5 w-3.5 text-soft-black-light" />}
                      secondaryLabel={<span className="text-sm text-teal font-medium underline">Forgot password?</span>}
                    />
                    <div className="h-10 rounded-lg bg-teal flex items-center justify-center mt-1">
                      <span className="text-warm-white font-semibold text-sm">Sign in</span>
                    </div>
                  </div>
                </div>
              </div>
          ...
          <MockAnnotations items={[
            { label: "Brand panel", rule: "Contextual messaging per route" },
            { label: "Trust signals", rule: "Checkmark + claim + detail pattern" },
            { label: "Form panel", rule: "Centered, max-w-md, semantic labels" },
            { label: "CTA hierarchy", rule: "Google first, email second, forgot last" },
          ]} />
        </div>
      </section>
      ...
      {/* ══════════════════════════════════════════════════════════════ */}
      <section className="bg-teal-dark px-6 sm:px-12 lg:px-24 py-32">
        <div className="max-w-5xl mx-auto">
          <SectionLabelLight>Brand Panel</SectionLabelLight>
          <h2 className="font-heading text-3xl sm:text-4xl text-warm-white mb-16">
      ...
      <section className="bg-warm-white px-6 sm:px-12 lg:px-24 py-32">
        <div className="max-w-5xl mx-auto">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {[
              { route: "/signup", heading: "Join NurseDex", tone: "Aspirational", message: "Value propositions for new users. Free, controlled, verified." },
              { route: "/login", heading: "Welcome back", tone: "Warm", message: "Community belonging. Local focus, privacy, free for families." },
              { route: "/forgot-password", heading: "Reset password", tone: "Reassuring", message: "\"No worries.\" Calm, confident, helpful. Reduce anxiety." },
              { route: "/signup/confirm", heading: "Check your inbox", tone: "Encouraging", message: "\"You are almost there.\" Momentum toward completion." },
              { route: "/reset-password", heading: "New password", tone: "Supportive", message: "\"Almost there.\" Quick resolution, back to normal." },
              { route: "/role-select", heading: "Choose your role", tone: "Guiding", message: "\"One quick step.\" Framing effort as minimal." },
            ].map((item) => (
              <div key={item.route} className="rounded-2xl border border-sage/20 overflow-hidden">
                <div className="bg-teal px-5 py-3">
                  <span className="font-mono text-xs text-warm-white/70">{item.route}</span>
                </div>
                <div className="px-5 py-5 space-y-3">
                  <h4 className="font-heading text-base text-soft-black">{item.heading}</h4>
                  <span className="inline-block bg-cream text-teal-dark font-body text-[10px] font-medium uppercase tracking-wider px-2.5 py-1 rounded-full">{item.tone}</span>
                  <p className="font-body text-xs text-soft-black-light leading-relaxed">{item.message}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      {/* ══════════════════════════════════════════════════════════════ */}
      <section className="bg-sage/10 px-6 sm:px-12 lg:px-24 py-32">
        <div className="max-w-5xl mx-auto">
      ...
      <section className="bg-soft-black px-4 sm:px-8 lg:px-16 py-16">
        <div className="max-w-3xl mx-auto">
              ...
              <div className="w-full max-w-sm">
                ...
                <div className="flex items-center gap-2 mb-6">
                  <div className="flex items-center gap-1.5">
                    ...
                    <span className="text-[10px] text-soft-black font-medium">Create account</span>
                  </div>
                  <div className="h-px w-6 bg-sage/30" />
                  <div className="flex items-center gap-1.5">
                    <div className="h-2 w-2 rounded-full bg-sage/40" />
                    <span className="text-[10px] text-soft-black-light">Confirm email</span>
                  </div>
                  <div className="h-px w-6 bg-sage/30" />
                  <div className="flex items-center gap-1.5">
                    <div className="h-2 w-2 rounded-full bg-sage/40" />
                    <span className="text-[10px] text-soft-black-light">Choose role</span>
                  </div>
                </div>
                <h2 className="font-heading text-xl mb-1">Join NurseDex</h2>
                <p className="text-xs text-soft-black-light mb-6">Create your free account. <span className="text-teal font-medium underline">Sign in instead</span></p>
                <div className="w-full h-10 rounded-lg border border-sage-dark/50 flex items-center justify-center gap-2 mb-5">
                  <GoogleLogo className="h-4 w-4" />
                  <span className="font-body text-sm font-semibold">Continue with Google</span>
                </div>
                ...
                <div className="space-y-3.5">
                  <MockInput label="Email" placeholder="your@email.com" />
                  <div>
                    <MockInput label="Password" placeholder="At least 8 characters" rightIcon={<Eye className="h-3.5 w-3.5 text-soft-black-light" />} />
                    <p className="text-[10px] text-soft-black-light mt-1">5/8 characters</p>
                  </div>
                  <p className="text-[10px] text-soft-black-light text-center">By clicking Join NurseDex, you agree to our <span className="text-teal underline">Terms</span> and <span className="text-teal underline">Privacy Policy</span>.</p>
                  <div className="h-10 rounded-lg bg-teal flex items-center justify-center mt-1">
                    <span className="text-warm-white font-semibold text-sm">Join NurseDex</span>
                  </div>
                </div>
              </div>
            </div>
          ...
          <MockAnnotations items={[
            { label: "Progress bar", rule: "3 dots + labels, dots-only on mobile" },
      ...
      <section className="bg-soft-black px-4 sm:px-8 lg:px-16 py-16">
        <div className="max-w-3xl mx-auto">
          ...
          <BrowserChrome url="nursedex.com/signup/confirm?email=jane@example.com">
            <div className="text-center px-10 py-16">
              ...
              <p className="text-soft-black-light text-xs leading-relaxed max-w-xs mx-auto">
                We sent a confirmation link to <strong className="text-soft-black">jane@example.com</strong>. Click the link to activate your account.
              </p>
              <div className="mt-5">
                <div className="inline-flex h-8 px-4 rounded-lg border border-sage-dark/50 items-center text-xs font-medium text-soft-black-light">
          ...
          <MockAnnotations items={[
            { label: "Mail icon", rule: "Centered, teal/10 circle, signals state change" },
            { label: "Email shown", rule: "Bold address confirms correct inbox" },
          ...
          <BrowserChrome url="nursedex.com/role-select">
            <div className="px-10 py-12">
              ...
              <div className="grid grid-cols-2 gap-4 mb-5">
                <div className="rounded-xl border-2 border-teal ring-2 ring-teal/20 p-5">
              ...
              <div className="h-10 rounded-lg bg-teal flex items-center justify-center">
                <span className="text-warm-white font-semibold text-sm">Continue</span>
              </div>
            </div>
          </BrowserChrome>
          <MockAnnotations items={[
            { label: "Cards", rule: "Large, tappable, side-by-side on desktop" },
      ...
      <section className="bg-soft-black px-4 sm:px-8 lg:px-16 py-16">
        <div className="max-w-5xl mx-auto">
          ...
          <div className="grid lg:grid-cols-2 gap-8">
            ...
            <div>
              <p className="font-body text-[10px] uppercase tracking-widest text-sage/50 mb-4">Step 1: Request</p>
              <BrowserChrome url="nursedex.com/forgot-password">
                <div className="px-8 py-12">
                  <h2 className="font-heading text-xl mb-1">Reset your password</h2>
                  <p className="text-xs text-soft-black-light mb-6">Enter your email and we will send you a reset link. <span className="text-teal font-medium underline">Back to sign in</span></p>
                  <div className="space-y-3.5">
                    <MockInput label="Email" placeholder="your@email.com" />
                    <div className="h-10 rounded-lg bg-teal flex items-center justify-center">
                      <span className="text-warm-white font-semibold text-sm">Send reset link</span>
                    </div>
                  </div>
                </div>
              </BrowserChrome>
            ...
            {/* Sent confirmation */}
            <div>
              <p className="font-body text-[10px] uppercase tracking-widest text-sage/50 mb-4">Step 2: Confirmation</p>
              <BrowserChrome url="nursedex.com/forgot-password/sent">
                <div className="text-center px-8 py-12">
                  ...
                  <h2 className="font-heading text-xl mb-2">Check your email</h2>
                  <p className="text-soft-black-light text-xs leading-relaxed max-w-xs mx-auto">If an account exists with that email, we sent a password reset link.</p>
                  <p className="mt-4 text-sm text-teal font-medium underline">Back to sign in</p>
                </div>
              </BrowserChrome>
            </div>
          </div>
        ...
        <div className="max-w-3xl mx-auto">
          ...
          <BrowserChrome url="nursedex.com/reset-password">
            <div className="px-10 py-12">
              ...
              <div className="space-y-3.5">
                <MockInput label="New password" placeholder="At least 8 characters" rightIcon={<Eye className="h-3.5 w-3.5 text-soft-black-light" />} />
                <div>
                  <MockInput label="Confirm password" placeholder="........" rightIcon={<EyeOff className="h-3.5 w-3.5 text-soft-black-light" />} />
                  <p className="text-[10px] text-error mt-1">Passwords do not match.</p>
                </div>
                <div className="h-10 rounded-lg bg-teal flex items-center justify-center mt-1">
                  <span className="text-warm-white font-semibold text-sm">Update password</span>
                </div>
              </div>
            </div>
          </BrowserChrome>
          <MockAnnotations items={[
            { label: "Independent toggles", rule: "Each field has its own eye icon" },
            { label: "Match validation", rule: "Client-side before submit" },
            { label: "Heading", rule: "\"Set new\" not \"Reset\" to match the action" },
          ]} />
        </div>
      </section>
      ...
      <section className="bg-soft-black px-4 sm:px-8 lg:px-16 py-16">
        <div className="max-w-3xl mx-auto">
          ...
          <div className="bg-warm-white rounded-xl overflow-hidden shadow-2xl p-8 max-w-sm mx-auto space-y-4">
            {/* Error with resend */}
            <div className="rounded-lg bg-error/10 px-4 py-3 text-sm text-error">
              Please confirm your email address before signing in.
              <button className="mt-1 block text-teal text-sm font-medium underline">Resend confirmation email</button>
            </div>
            {/* Field error */}
            <div>
              <div className="h-10 rounded-lg border-2 border-error px-3 flex items-center">
                <span className="text-sm text-soft-black">bad@</span>
              </div>
          ...
          <MockAnnotations items={[
            { label: "Block errors", rule: "Inline, near the problem, with recovery action" },
            { label: "Field errors", rule: "Red border + message below, aria-describedby" },
          ...
          <div className="grid sm:grid-cols-2 gap-8">
            {accessibilityDecisions.map((item) => (
              <div key={item.label} className="flex items-start gap-3">
                <span className="w-5 h-5 rounded-full bg-warm-white/10 flex items-center justify-center shrink-0 mt-0.5">
                  <svg className="w-3 h-3 text-cream" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={3}>
                    <path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" />
                  </svg>
                </span>
                <div>
                  <h4 className="font-body text-sm font-medium text-warm-white mb-1">{item.label}</h4>
                  <p className="font-body text-xs text-sage-light leading-relaxed">{item.detail}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="max-w-5xl mx-auto">
          ...
          <div className="grid sm:grid-cols-2 gap-10">
            {mobileDecisions.map((item) => (
              <div key={item.label}>
                <h4 className="font-heading text-lg text-teal-dark mb-2">{item.label}</h4>
                <p className="font-body text-soft-black-light text-sm leading-relaxed">{item.detail}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="bg-sage/10 px-6 sm:px-12 lg:px-24 py-32">
        <div className="max-w-5xl mx-auto">
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {/* Google button */}
            <div>
              <p className="font-body text-[10px] uppercase tracking-widest text-sage-dark mb-4">OAuth button</p>
              <div className="h-11 rounded-lg border border-sage-dark/50 bg-warm-white shadow-sm flex items-center justify-center gap-2">
                <GoogleLogo />
                <span className="font-body text-base font-semibold">Continue with Google</span>
              </div>
              <p className="font-body text-xs text-soft-black-light mt-3">Official multicolor logo. border-sage-dark/50 for visibility on warm-white.</p>
            </div>
            {/* Primary button states */}
            <div>
              <p className="font-body text-[10px] uppercase tracking-widest text-sage-dark mb-4">Primary action</p>
              <div className="h-11 rounded-lg bg-teal flex items-center justify-center">
                <span className="text-warm-white font-semibold text-base">Sign in</span>
              </div>
              <div className="h-11 rounded-lg bg-teal/50 flex items-center justify-center gap-2 mt-3">
                <Loader2 className="h-4 w-4 text-warm-white animate-spin" />
                <span className="text-warm-white font-semibold text-base">Signing in...</span>
              </div>
              <p className="font-body text-xs text-soft-black-light mt-3">Full-width, h-11, teal bg. Loading disables + shows spinner.</p>
            </div>
            {/* Separator */}
            <div>
              <p className="font-body text-[10px] uppercase tracking-widest text-sage-dark mb-4">Separator</p>
              <div className="relative py-6">
                <div className="h-px bg-sage/20" />
                <span className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 bg-warm-white px-3 text-xs text-soft-black-light">or continue with email</span>
              </div>
              <p className="font-body text-xs text-soft-black-light mt-3">Text centered over rule. bg-background covers the line.</p>
            </div>
          </div>
          ...
          <div className="grid md:grid-cols-3 gap-8 mt-12">
            {/* Role cards */}
            <div>
              <p className="font-body text-[10px] uppercase tracking-widest text-sage-dark mb-4">Role card (selected)</p>
              <div className="rounded-xl border-2 border-teal ring-2 ring-teal/20 p-4">
                <Stethoscope className="h-5 w-5 text-teal mb-1.5" />
                <p className="font-heading text-sm font-semibold text-soft-black">I am a nurse</p>
              </div>
              <p className="font-body text-xs text-soft-black-light mt-3">border-teal + ring-teal/20 glow on selection.</p>
            </div>
            {/* Confirmation icon */}
            <div>
              <p className="font-body text-[10px] uppercase tracking-widest text-sage-dark mb-4">Confirmation icon</p>
              <div className="flex justify-center">
                <div className="h-14 w-14 rounded-full bg-teal/10 flex items-center justify-center">
                  <Mail className="h-7 w-7 text-teal" />
                </div>
              </div>
              <p className="font-body text-xs text-soft-black-light mt-3">Centered icon in teal/10 circle. Signals state transition.</p>
            </div>
            {/* Progress indicator */}
            <div>
              <p className="font-body text-[10px] uppercase tracking-widest text-sage-dark mb-4">Progress indicator</p>
              <div className="flex items-center gap-2 py-3">
                <div className="flex items-center gap-1.5">
                  <div className="h-2 w-2 rounded-full bg-teal" />
                  <span className="text-[10px] text-soft-black font-medium">Step 1</span>
                </div>
                <div className="h-px w-6 bg-sage/30" />
                <div className="flex items-center gap-1.5">
                  <div className="h-2 w-2 rounded-full bg-sage/40" />
                  <span className="text-[10px] text-soft-black-light">Step 2</span>
                </div>
                <div className="h-px w-6 bg-sage/30" />
                <div className="flex items-center gap-1.5">
                  <div className="h-2 w-2 rounded-full bg-sage/40" />
                  <span className="text-[10px] text-soft-black-light">Step 3</span>
                </div>
              </div>
              <p className="font-body text-xs text-soft-black-light mt-3">Dot + label. Active dot is teal, inactive sage/40.</p>
            </div>
          </div>
        </div>
      </section>
            ...
            {[
              {
                ...
                detail: "Google OAuth or email + password. Minimal fields. No name collection at this stage to reduce friction. Terms acceptance is implicit (visible link, hidden checkbox).",
              },
              {
                step: "02",
                title: "Confirm email",
                detail: "Redirect to /signup/confirm with the email pre-displayed. Clear instruction to check inbox. Resend link available with 60-second cooldown. No dead ends.",
              },
              {
                step: "03",
                title: "Choose role",
                detail: "Family or Nurse. This single choice tailors the entire product experience. Presented as large, tappable cards with icons. No skip option, this is essential data.",
              },
            ].map((item, i) => (
              <div key={item.step} className={`px-8 py-10 ${i < 2 ? "border-b md:border-b-0 md:border-r border-sage/20" : ""}`}>
                <span className="font-heading text-4xl text-sage/30">{item.step}</span>
                <h4 className="font-heading text-xl text-teal-dark mt-3 mb-3">{item.title}</h4>
                <p className="font-body text-sm text-soft-black-light leading-relaxed">{item.detail}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="bg-soft-black px-4 sm:px-8 lg:px-16 py-16">
        <div className="max-w-5xl mx-auto">
          <p className="font-body text-xs uppercase tracking-[0.4em] text-sage/40 mb-4 text-center">
            Interactive Sandbox
          </p>
          <h2 className="font-heading text-3xl sm:text-4xl text-warm-white mb-4 text-center">
            Try it yourself
          </h2>
          <p className="font-body text-sage text-sm text-center max-w-lg mx-auto mb-16">
            Fully interactive versions of every auth page. Type, click, toggle, and submit.
            Nothing is connected to a real backend. All behavior is simulated locally.
          </p>
      ...
      <section className="bg-teal-dark px-6 sm:px-12 lg:px-24 py-32">
        <div className="max-w-5xl mx-auto flex flex-col items-center text-center gap-8">
          ...
          <h3 className="font-heading text-3xl sm:text-4xl text-warm-white">Authentication Design</h3>
          <p className="font-body text-sage text-lg max-w-md">Every screen is a chance to build trust. The auth flow is where that trust begins.</p>
          <div className="w-8 h-0.5 bg-sage/30 rounded-full" />
          <p className="font-body text-sm text-sage/60 italic">NurseDex Design System &middot; Auth Module</p>
          <p className="font-body text-xs tracking-widest text-sage/40 uppercase">v1.0 &middot; March 2026</p>
        </div>
      </section>
    </div>
  );
